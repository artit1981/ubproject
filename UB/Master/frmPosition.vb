...
Public Class frmPosition
    Inherits iEditForm
    Dim mcls As New PositionDAO
    Private Const mFormName As String = "frmPosition"
...
#Region "Private"
    Private Sub SetComboDepartment()
        Try
            SetLookUpDepartment(cboDepartment)

        Catch e As Exception
            Err.Raise(Err.Number, e.Source, mFormName & ".SetComboDepartment : " & e.Message)
        Finally
        End Try
    End Sub
        ...
        Try
            ...
            ElseIf pMode = DataMode.ModeEdit Then
                If mcls.InitailData(pID) Then
                    ...
                    cboDepartment.EditValue = mcls.DepartmentID
    ...
    Private Function Verify() As Boolean
        ...
        Try
            ...
            If ConvertNullToZero(cboDepartment.GetColumnValue("ID")) <= 0 Then
                SetErrorProvider(DxErrorProvider1, cboDepartment, "กรุณาระบุข้อมูล")
            End If
...
#Region "Overrides"
    ...
    Protected Overrides Function Save(ByVal pMode As Integer, ByVal pID As Long) As Boolean
        ...
        Try
            ...
            mcls.DepartmentID = ConvertNullToZero(cboDepartment.GetColumnValue("ID"))
