...
#End Region
    ...
    Public Function CHECKSUM_AGG() As Long
        ...
        Catch e As Exception
            Err.Raise(Err.Number, e.Source, "StockInDAO.CHECKSUM_AGG : " & e.Message)
            Return 0
        Finally
            dataTable = Nothing
        End Try
    ...
    Public Overrides Function InitailData(ByVal pID As Long, Optional ByVal pName As String = "", Optional ByVal tr As SqlTransaction = Nothing) As Boolean
        ...
        Try
            ...
            If dataTable.Rows.Count > 0 Then
                For Each dr As DataRow In dataTable.Rows
                    ...
                    RefOrderIDs = ConvertNullToZero(dr("RefOrderID"))
        ...
        Catch e As Exception
            Err.Raise(Err.Number, e.Source, "StockInDAO.InitailData : " & e.Message)
            Return False
        Finally
            lUserDAO = Nothing
            dataTable = Nothing
    ...
    Public Function GetInfoHtml() As String
        ...
        'Catch e As Exception
        '    Err.Raise(Err.Number, e.Source, "StockInDAO.GetInfoHtml : " & e.Message)
    ...
    Public Function GetDataTable(ByVal pID As Long, ByVal pOnlyActive As Boolean, Optional ByVal pOtherCondition As String = "", Optional ByRef ptr As SqlTransaction = Nothing) As DataTable
        ...
        Try
            ...
            SQL = SQL & ",Orders.IsDelete,Orders.Remark"
            SQL = SQL & " FROM Orders  "
            SQL = SQL & " LEFT OUTER JOIN Employee ON Orders.EmpID=Employee.EmpID  "
            'SQL = SQL & " LEFT OUTER JOIN Product_LocationDTL ON StockIn.LocationDTLID=Product_LocationDTL.LocationDTLID  "
            SQL = SQL & " WHERE Orders.TableID=" & MasterType.StockIn
        ...
        Catch e As Exception
            Err.Raise(Err.Number, e.Source, "StockInDAO.GetDataTable : " & e.Message)
        End Try
        Return dataTable
    End Function
    ...
    Public Overrides Function SaveData(Optional ByRef ptr As SqlTransaction = Nothing) As Boolean
        ...
        Try
            ...
            Select Case ModeData
                Case DataMode.ModeNew
                    ...
                    SQL = " INSERT INTO Orders  (OrderID,OrderCode,OrderDate,EmpID,RefOrderID"
                    SQL = SQL & ",Remark,CreateBy,CreateTime,IsInActive,IsDelete,ImportTXID,TableID" ',LocationDTLID "
                    SQL = SQL & " )"
                    SQL = SQL & " VALUES ( @IDs"
                    SQL = SQL & " ,  @OrderCode"
                    SQL = SQL & " ,  @OrderDate"
            ...
            myCommand.Parameters.Add(New SqlParameter("@RefOrderID", ConvertNullToZero(RefOrderIDs)))
            ...
            ElseIf ProductDAOs.Count > 0 Then
                For Each ProductList As ProductListDAO In ProductDAOs
                    ...
                    lclsCost.SaveData(ProductList.ProductID, ProductCostDAO.CostTypes.Average, ProductList.Cost, ProductList.ID, tr)
                Next
            End If

                'Ref Order 
            If RefOrderIDs > 0 Then
                If ModeData = DataMode.ModeNew Then
                    UpdateRefOrderStatus(ID, RefOrderIDs, "Close", tr)
                    SetFlagProductList(ProductDAOs, True, RefOrderIDs, tr)
                ElseIf ModeData = DataMode.ModeDelete Then
                    UpdateRefOrderStatus(0, RefOrderIDs, "Open", tr)
                    SetFlagProductList(ProductDAOs, False, RefOrderIDs, tr)
                End If
            ...
            If ModeData = DataMode.ModeNew Then
                UpdateLastID(MasterType.StockIn, tr)
            End If
            InsertActivity(ModeData, MasterType.StockIn, Code, tr)
        ...
        Catch e As Exception
            ...
            Err.Raise(Err.Number, e.Source, "StockInDAO.SaveData : " & e.Message)
            Return False
        End Try
    ...
    Public Overrides Function CheckExist() As Boolean
        ...
        Catch e As Exception
            Err.Raise(Err.Number, e.Source, "StockInDAO.CheckExist : " & e.Message)
            Return False
        Finally
            dataTable = Nothing
        End Try
    ...
    Public Function CheckNotExist() As Boolean ''ถูกใช้งานอยู่ ???
        ...
        Try
            ...
            'Return dataTable.Rows.Count > 0
        Catch e As Exception
            Err.Raise(Err.Number, e.Source, "StockInDAO.CheckNotExist : " & e.Message)
        End Try
        Return False
    End Function
